...
Процедура ДлительнаяОперацияМассовоеЗаполнениеАктов(Параметры) Экспорт
...
Функция ПолучитьЦветRGB(КодЦвета) Экспорт

	Красный = КодЦвета % 256;
	Зеленый = (Цел(КодЦвета / 256)) % 256;
	Синий = (Цел(КодЦвета / 256 / 256)) % 256;
	
	RGB = Новый Цвет(Красный, Зеленый, Синий);
