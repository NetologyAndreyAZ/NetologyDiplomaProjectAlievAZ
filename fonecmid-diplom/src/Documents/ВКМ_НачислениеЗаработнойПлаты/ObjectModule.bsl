...
Процедура СформироватьДополнительныеНачисления(МинимальнаяДатаНачала, МаксимальнаяДатаОкончания);
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ ВТ_НачислениеЗаработнойПлаты
		|ИЗ
		...
		|	СУММА(ЕСТЬNULL(ВКМ_ВыполненныеСотрудникомРаботыОстатки.СуммаКОплатеОстаток, 0)) КАК СуммаКОплате,
		|	ВТ_НачислениеЗаработнойПлаты.Сотрудник.Категория КАК Категория
		|ИЗ
		|	ВТ_НачислениеЗаработнойПлаты КАК ВТ_НачислениеЗаработнойПлаты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ВКМ_ВыполненныеСотрудникомРаботы.Остатки(&Период, ) КАК ВКМ_ВыполненныеСотрудникомРаботыОстатки
		|		ПО ВТ_НачислениеЗаработнойПлаты.Сотрудник = ВКМ_ВыполненныеСотрудникомРаботыОстатки.Сотрудник
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_НачислениеЗаработнойПлаты.Сотрудник.Категория,
		|	ВКМ_ВыполненныеСотрудникомРаботыОстатки.Сотрудник";
