...
Процедура ЗаполнитьНаСервере() Экспорт
	...
	Обработки.ВКМ_МассовоеСозданиеАктов.ДлительнаяОперацияМассовоеЗаполнениеАктов(Выборка, ДанОбъект, ДанОбъектыТЧ);
...
Функция ПолучитьВыборку()
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ВКМ_РеализацияДоговор.Период = &Дата";
	
	Дата = Дата(Объект.Год, ВКМ_ОбслуживаниеКлиентов.НомерМесяца(Объект.Месяц), 1);
...
Функция ВыполнитьФоновоеЗаданиеНаСервере(ПараметрыЗапуска, УникальныйИдентификатор)
	...
	ВыполняемыйМетод = "ВКМ_ОбслуживаниеКлиентов.ДлительнаяОперацияМассовоеЗаполнениеАктов";	
	...
	СтруктураФоновогоЗадания = ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения, ВыполняемыйМетод, ПараметрыЗапуска);
